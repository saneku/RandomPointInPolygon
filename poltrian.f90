function angle_degree ( x1, y1, x2, y2, x3, y3 )
  ...
  subroutine my_polygon_triangulate ( n, x, y, triangles,triangles_areas)
  ...
  !
    ...
    integer ( kind = 4 ) prev_node(n)
    integer ( kind = 4 ) triangle_num
    integer ( kind = 4 ) triangles(4,n-2)
    real ( kind = 8 ) triangles_areas(n-2)    
    real ( kind = 8 ) x(n)
    real ( kind = 8 ) y(n)
  ...
  !
        ...
        triangles_areas(triangle_num)=triangle_area ( x(i3), y(i3), x(i1), y(i1), x(i2), y(i2) )
